...
class DLL_exp Keithlay2410	
{
	RQ_OBJECT("Keithlay2410")
public:
	 Keithlay2410(const char *Port); // Constructor: it needs to know the name of the port. up to now there is only one constructor defined. 
	virtual  ~Keithlay2410(void);  // the Destructor has no special implementation but by default it closes all open files. 
...
private:
	Keithlay2410(const Keithlay2410&); // disabling the copy constructor since one cannot copy the com ports
	Keithlay2410& operator=(const Keithlay2410&); //disabling assign for the same reason.
...
ClassDef(Keithlay2410, 1);//needed for the ROOT dict
